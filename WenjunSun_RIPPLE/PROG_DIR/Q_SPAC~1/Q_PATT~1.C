/*    
**	Created by WENJUN SUN at Carnegie Mellon University
**
...
char *argv[];
{
	...
	fscanf(ofpi,"%f %f %f %f",&Amp,&x0,&Rhm,&Zh);
	gamma = gamma*radcon;
	real_con = INCH / d;	/* d -----> 1 inch */
	...
	if(Zh > 1.0) { /* Draw bilayer */
		...
		fprintf(ofpo,"newpath\n");
		x=real_sp.llx;
		y=real_sp.lly+Zh*real_con;
		fprintf(ofpo,"%.3g i %.3g i moveto\n",x,y);
		fprintf(ofpo,"%.3g i %.3g i rlineto\n",x0*real_con,Amp*real_con);
		fprintf(ofpo,"%.3g i %.3g i rlineto\n",(rlam-x0)*real_con,-Amp*real_con);
		x=0.0;
		fprintf(ofpo,"%.3g i %.3g i rlineto\n",x,-2.0*Zh*real_con);
		fprintf(ofpo,"%.3g i %.3g i rlineto\n",(x0-rlam)*real_con,Amp*real_con);
		fprintf(ofpo,"%.3g i %.3g i rlineto\n",-x0*real_con,-Amp*real_con);
		fprintf(ofpo,"closepath 0.75 setgray fill\n");
		fprintf(ofpo,"grestore\n\n");
	}
	...
	if(Amp > 1.0){
		...
		fprintf(ofpo,"%.3f i %.3f i rlineto\n",x0*real_con,Amp*real_con);
		fprintf(ofpo,"%.3f i %.3f i lineto\n",real_sp.lrx,real_sp.lry);
		fprintf(ofpo,"%g setgray %d setlinewidth stroke\n",
	...
	/* left side */
		x=real_sp.llx+0.5*x0real_con;
		y=real_sp.lly+0.5*Amp*real_con;
		slope=alpha/radcon;
		length=2.0;
		fprintf(ofpo,"%.3g i %.3g i %.3g %.3g i normal\n",x,y,slope,length);
	/* right side */
		x=real_sp.lrx-0.5*(rlam-x0)*real_con;
		y=real_sp.lry+0.5*Amp*real_con;
		slope=-beta/radcon;
		length=2.0;
		fprintf(ofpo,"%.3g i %.3g i %.3g %.3g i normal\n",x,y,slope,length);
	...
	if (Zh > 1.0) {
		fprintf(ofpo,"getCou (Rhm = %g, Zh = %gA) show\n",Rhm,Zh);
		fprintf(ofpo,"newline\n");
	}
