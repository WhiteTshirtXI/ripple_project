...
char *argv[];
{
	...
	fscanf(ofpi,"%f %f %f %f %f",&Amp,&x0,&Rhm,&Zh,&tilt);
	gamma = gamma*radcon;
	tilt=tilt*radcon;
	...
	if(Zh > 1.0) { /* Draw bilayer */
		...
		bilayer(ofpo,x,y,rlam,x0,real_con,x0_con,Amp_con,Zh_con,tilt);
	...
	if(Amp > 1.0){
		x=0.5*(real_sp.llx+real_sp.ulx);
		y=0.5*(real_sp.lly+real_sp.uly);
		profile(ofpo,x,y,rlam,x0,real_con,x0_con,Amp_con,Zh_con,tilt);
	...
	fprintf(ofpo,"newpath\n");
	fprintf(ofpo,"%.3f i %.3f i moveto\n",
		real_sp.llx-rlam*real_con,real_sp.lly);
	fprintf(ofpo,"%.3f i %.3f i lineto\n",
		real_sp.ulx-rlam*real_con,real_sp.uly);
	fprintf(ofpo,"%.3f i %.3f i lineto\n",
		real_sp.urx-rlam*real_con,real_sp.ury);
	fprintf(ofpo,"%.3f i %.3f i lineto\n",
		real_sp.lrx-rlam*real_con,real_sp.lry);
	fprintf(ofpo,"closepath %d setlinewidth stroke\n\n",LINEWIDTH);
...
bilayer (ofp,x,y,rlam,x0,real_con,x0_con,Amp_con,Zh_con,tilt)
FILE *ofp;
float x,y;
...
profile (ofp,x,y,rlam,x0,real_con,x0_con,Amp_con,Zh_con,tilt)
FILE *ofp;
float x,y;
float rlam,x0,real_con;
float x0_con,Amp_con,Zh_con;
