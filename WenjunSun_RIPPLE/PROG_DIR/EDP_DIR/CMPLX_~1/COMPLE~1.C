/* 
**	Program: complex_ripple_edp.c
**
**		By Wenjun Sun, 8/27/95, Carnegie Mellon University
**
**	Purpose: To reconstruct the 2-D electron density
**	profile using the experimental amplitudes of the
...
**			indices and the complex form factors of 
**			ripple scattering peaks.
**
**	Output:
...
char *argv[];
{
	...
	for(m=0;m<Nmax;++m){
		fscanf(ifp,"%d %d %f %f",&h,&k,&Fhk_r,&Fhk_i);
		qx=2.0*M_PI*k/rlam;
	...
	for (i=0; i < pres; i++) {
		for (m=0; m < (pres-24); m=m+25) { 
			for (j=m; j < (m+25); j++) { 
/*				gs = 255.0*(1.0-(p[j][i]-gsmin)/(gsmax-gsmin));*/
